function process_behave_data_v2(behav,design,ctch)
% Future Updates:
...
for s=1:length(behav.subjects)
    ...
    A_R1=sum(R1_HCO)/(sum(R1_HCO)+sum(R1_HCM));
    B_R1=sum(R1_HCFA)/(sum(R1_HCFA)+sum(R1_HCCR));
    C_R1=sum(R1_HCCR)/(sum(R1_HCCR)+sum(R1_HCFA));
    D_R1=sum(R1_HCM)/(sum(R1_HCM)+sum(R1_HCO));
    ...
    A_R2=sum(R2_HCO)/(sum(R2_HCO)+sum(R2_HCM));
    B_R2=sum(R2_HCFA)/(sum(R2_HCCR)+sum(R2_HCFA));
    C_R2=sum(R2_HCCR)/(sum(R2_HCCR)+sum(R2_HCFA));
    D_R2=sum(R2_HCM)/(sum(R2_HCO)+sum(R2_HCM));
    ...
    E_R1=sum(R1_LCO)/(sum(R1_LCO)+sum(R1_LCM));
    F_R1=sum(R1_LCFA)/(sum(R1_LCCR)+sum(R1_LCFA));
    G_R1=sum(R1_LCCR)/(sum(R1_LCCR)+sum(R1_LCFA));
    H_R1=sum(R1_LCM)/(sum(R1_LCO)+sum(R1_LCM));
    ...
    E_R2=sum(R2_LCO)/(sum(R2_LCO)+sum(R2_LCM));
    F_R2=sum(R2_LCFA)/(sum(R2_LCCR)+sum(R2_LCFA));
    G_R2=sum(R2_LCCR)/(sum(R2_LCCR)+sum(R2_LCFA));
    H_R2=sum(R2_LCM)/(sum(R2_LCO)+sum(R2_LCM));
