...
% psuedo-standard. The structure should have an area for a .header and .col
% feild. This is requisite. Essentially the format should mirror that of an
...
%
function write_struct_txt(data,loc,varargin)
...
if nohead==0
    for i=1:length(data)
        ...
        switch current_class
            case 'cell'
                fprintf(fid,[char(data{i}.header) delim]);
            case 'char'
                fprintf(fid,[data{i}.header delim]);
            otherwise
                fprintf(fid,[num2str(data{i}.header) delim]);
        end
    end
    fprintf(fid,'\n');
end
